...
describe("BuzzVaultExponential Tests", () => {
    ...
    describe("buyNative", () => {
        ...
        it("should handle multiple buys in succession", async () => {
            ...
            await expVault
                .connect(user2Signer)
                .buyNative(token.address, ethers.utils.parseEther("0.001"), ethers.constants.AddressZero, user2Signer.address, { value: ethers.utils.parseEther("0.01") });
        ...
        it("should revert if msg.value is zero", async () => {
            await expect(
                expVault.buyNative(token.address, ethers.utils.parseEther("1"), ethers.constants.AddressZero, ownerSigner.address, { value: 0 })
            ).to.be.revertedWithCustomError(expVault, "BuzzVault_QuoteAmountZero");
        });
        it("should revert if base token is not WBera", async () => {
            await expect(
                expVault.buyNative(ownerSigner.address, ethers.utils.parseEther("1"), ethers.constants.AddressZero, ownerSigner.address, {
    ...
    describe("_buyTokens", () => {
        it("should revert if user wants less than 0.001 token min", async () => {
            await expect(
                expVault.buyNative(token.address, ethers.utils.parseEther("0.0001"), ethers.constants.AddressZero, ownerSigner.address, {
        ...
        it("should revert if token doesn't exist", async () => {
            ...
            await expect(expVault.buy(wBera.address, ethers.utils.parseEther("1"), ethers.utils.parseEther("1"), ethers.constants.AddressZero, ownerSigner.address)).to
                .reverted;
            // fails in safeTransferFrom
        });
        it("should revert if the recipient is the zero address", async () => {
            await expect(expVault.buyNative(token.address, ethers.utils.parseEther("0.001"), ethers.constants.AddressZero, ethers.constants.AddressZero, { value: ethers.utils.parseEther("0.1") })).to.be.revertedWithCustomError(expVault, "BuzzVault_ZeroAddressRecipient");
        })
        it("should revert if token is already listed to Bex", async () => {
            ...
            await wBera.approve(expVault.address, ethers.utils.parseEther("1"));
            await expect(
                expVault.buyNative(token.address, ethers.utils.parseEther("1000"), ethers.constants.AddressZero, ownerSigner.address, {
                    value: ethers.utils.parseEther("1500"),
                })
        ...
        it("should revert if reserves are invalid", async () => {
            await expect(
                expVault.buyNative(token.address, ethers.utils.parseEther("1000000000000000000"), ethers.constants.AddressZero, ownerSigner.address, {
                    value: ethers.utils.parseEther("0.1"),
                })
        ...
        it("should revert if user will get less than 0.001 token", async () => {
            await expect(
                expVault.buyNative(token.address, ethers.utils.parseEther("0.001"), ethers.constants.AddressZero, ownerSigner.address, {
    ...
    describe("sell", () => {
        beforeEach(async () => {
            await expVault
                .connect(user1Signer)
                .buyNative(token.address, ethers.utils.parseEther("3"), ethers.constants.AddressZero, user1Signer.address, { value: ethers.utils.parseEther("3") });
        ...
        it("should revert if the token amount is zero", async () => {
            await expect(expVault.sell(ownerSigner.address, 0, 0, ethers.constants.AddressZero, ownerSigner.address, false)).to.be.revertedWithCustomError(
                expVault,
        ...
        it("should revert if the token amount to sell is less than the MIN_TOKEN_AMOUNT", async () => {
            ...
            await expect(
                expVault.sell(ownerSigner.address, MIN_TOKEN_AMOUNT.sub(1), 0, ethers.constants.AddressZero, ownerSigner.address, false)
            ).to.be.revertedWithCustomError(expVault, "BuzzVault_InvalidMinTokenAmount");
        });
        it("should revert if token doesn't exist", async () => {
            await expect(
                expVault.sell(ownerSigner.address, ethers.utils.parseEther("1"), 0, ethers.constants.AddressZero, ownerSigner.address, false)
        ...
        it("should revert if token is already listed to Bex", async () => {
            ...
            await expect(
                expVault.sell(token.address, ethers.utils.parseEther("2"), ethers.utils.parseEther("2"), ethers.constants.AddressZero, ownerSigner.address, false)
            ).to.be.revertedWithCustomError(expVault, "BuzzVault_BexListed");
        });
        it("should revert if user balance is invalid", async () => {
            await expect(
                expVault.sell(token.address, ethers.utils.parseEther("10000000000000000000000"), 0, ethers.constants.AddressZero, ownerSigner.address, false)
            ).to.be.revertedWithCustomError(expVault, "BuzzVault_InvalidUserBalance");
        });
        it("should set a referral if one is provided", async () => {
            await expVault
                .connect(user1Signer)
                .sell(token.address, ethers.utils.parseEther("10000"), ethers.utils.parseEther("0.0001"), ownerSigner.address, user1Signer.address, false);
            expect(await referralManager.referredBy(user1Signer.address)).to.be.equal(ownerSigner.address);
        });
        ...
        it("should transfer the 1% of msg.value to treasury", async () => {
            ...
            await expVault.connect(user1Signer).sell(token.address, sellAmount, ethers.utils.parseEther("0.0001"), ethers.constants.AddressZero, user1Signer.address, false);
        ...
        it("should not collect a referral fee if trading fee is 0", async () => {
            ...
            await expVault
                .connect(user1Signer)
                .sell(token.address, ethers.utils.parseEther("10000"), ethers.utils.parseEther("0.0001"), ethers.constants.AddressZero, user1Signer.address, false);
            const treasuryBalanceAfter = await wBera.balanceOf(treasury.address);
        ...
        it("should transfer quote tokens from the user", async () => {
            ...
            await expVault.connect(user1Signer).sell(token.address, amountToSell, ethers.utils.parseEther("0.0001"), ownerSigner.address, user1Signer.address, false);
            const userBalanceAfter = await token.balanceOf(user1Signer.address);
        ...
        it("should revert if user wants to sell less than 0.0001 token", async () => {
            await expect(
                expVault.sell(token.address, ethers.utils.parseEther("0.00001"), 0, ethers.constants.AddressZero, ownerSigner.address, false)
            ).to.be.revertedWithCustomError(expVault, "BuzzVault_InvalidMinTokenAmount");
        });
        it("should emit a trade event", async () => {
            const userTokenBalance = await token.balanceOf(user1Signer.address);
        ...
        it("should revert if the recipient is the zero address", async () => {
            const userTokenBalance = await token.balanceOf(user1Signer.address);
            await token.connect(user1Signer).approve(expVault.address, userTokenBalance);
            await expect(expVault.connect(user1Signer).sell(token.address, userTokenBalance, 0, ethers.constants.AddressZero, ethers.constants.AddressZero, false)).to.be.revertedWithCustomError(expVault, 
                "BuzzVault_ZeroAddressRecipient"
            );
        });
        it("should transfer the tokens to the recipient, if it's a different address than the caller", async () => {
            const userBalance = await wBera.balanceOf(ownerSigner.address);
            const userTokenBalance = await token.balanceOf(user1Signer.address);
            await token.connect(user1Signer).approve(expVault.address, userTokenBalance);
            await expVault.connect(user1Signer).sell(token.address, userTokenBalance, 0, ethers.constants.AddressZero, ownerSigner.address, false);
            expect(await wBera.balanceOf(ownerSigner.address)).to.be.greaterThan(userBalance);
        });
        it("should emit the recipient address as the seller", async () => {
            const userTokenBalance = await token.balanceOf(user1Signer.address);
            await token.connect(user1Signer).approve(expVault.address, userTokenBalance);
            expect(await expVault.connect(user1Signer).sell(token.address, userTokenBalance, 0, ethers.constants.AddressZero, ownerSigner.address, false))
                .to.emit(expVault, "Trade")
                .withArgs(
                    ownerSigner.address,
                    token.address,
    ...
    describe("pause", () => {
        ...
        it("should not allow calling buy", async () => {
            ...
            await wBera.deposit({ value: ethers.utils.parseEther("1") });
            await wBera.connect(ownerSigner).approve(expVault.address, ethers.utils.parseEther("1"));
            await expect(expVault.buy(token.address, ethers.utils.parseEther("1"), ethers.utils.parseEther("1"), ethers.constants.AddressZero, ownerSigner.address)).to.be.revertedWith("Pausable: paused");
        });
