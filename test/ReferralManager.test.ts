...
describe("ReferralManager Tests", () => {
    ...
    describe("setReferral", () => {
        ...
        it("should not set the referral if it's the same as msg.sender", async () => {
            await expVault.connect(user1Signer).buyNative(token.address, ethers.utils.parseEther("0.001"), user1Signer.address, user1Signer.address, {
                value: ethers.utils.parseEther("0.01"),
            });
        ...
        describe("setReferral - indirect referral", () => {
            beforeEach(async () => {
                tx = await expVault.connect(user2Signer).buyNative(token.address, ethers.utils.parseEther("0.001"), ownerSigner.address, user2Signer.address, {
                    value: ethers.utils.parseEther("0.01"),
                });
            });
    ...
    describe("getReferralBpsFor", () => {
        ...
        describe("getReferralBpsFor - indirect referral", () => {
            beforeEach(async () => {
                await expVault.connect(user2Signer).buyNative(token.address, ethers.utils.parseEther("0.001"), ownerSigner.address, user2Signer.address, {
                    value: ethers.utils.parseEther("0.01"),
                });
            });
